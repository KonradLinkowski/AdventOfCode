...
function printStack(stack) {
    ...
    for (let i = 0; i < stack.length; i++) {
        temp += '-> ';
        temp += stack[i].name + ' ' + stack[i].weight + ' ';
    }
    console.log(temp);
}

function treeTraversal(root, array) {
    function go(el, stack) {
        let weight = calculateWeight(el, array);
        stack.push({ name: el, weight: weight });
        printStack(stack);
        for (let i = 0; i < array.length; i++) {
            if (el == array[i][0]) {
                for (let j = 2; j < array[i].length; j++) {
...
function findWrongWeight(root, array) {
    function go(el) {
        let weight = calculateWeight(el, array);
        for (let i = 0; i < array.length; i++) {
            if (el == array[i][0]) {
                ...
                for (let j = 2; j < array[i].length; j++) {
                    if (j == 2) {
                        standard = go(array[i][j]);
                    }
                    let helper = go(array[i][j]);
                    if (helper != standard) {
                        console.log("Wrong element", array[i][j], "weight", helper, "proper weight", standard);
                    }
                }
            }
        }
...
function calculateWeight(element, array) {
    function calc(el) {
        let temp;
        for (let i = 0; i < array.length; i++) {
            if (array[i][0] == el) {
                ...
                let weight = 0;
                for (let j = 2; j < array[i].length; j++) {
                    weight += calc(array[i][j]);
                }
                return array[i][1] + weight;
                break;
            }
        }
        return temp[1];
...
findWrongWeight(findRoot(words[0][0], words), words);
//treeTraversal(findRoot(words[0][0], words), words);
/*
for (let i = 0; i < words.length; i++) {
    console.log(words[i][0], calculateWeight(words[i][0], words));
}
*/